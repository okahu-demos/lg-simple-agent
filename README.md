...
Fork this repo then run the python agent app [`lg_travel_agent.py`](lg_travel_agent.py) to ask questions.  
...
|---------|---------------|-------------|
| Run the agent | OpenAI subscription and API key | 1. Get your API key from [OpenAI developer platform](https://platform.openai.com/overview)<br> 2. In VS Code <br> - Configure `OPENAI_API_KEY` in [`.env`](.env) <br> - Set `MONOCLE_EXPORTER=file` in [`.env`](.env) |
...
| In the cloud <br> - Run evals with cloud compute<br> - Manage traces in the cloud | Okahu Cloud tenant and API key | 1. Provision [Okahu AI Observability Cloud](http://portal.okahu.co/) with your `Github ID` using the IDE extension<br> Set `MONOCLE_EXPORTER=file,okahu` for both local and cloud traces |
...
- Open `lg_travel_agent.py`
- Select Okahu extension 
    ![Okahu AI Observability extension icon](images/okahu_vs_code_icon.png)
- Clink on the 'Instrument using Monocle2AI'
- Notice it added following lines to the file
  ```python
...
### Rerun the instrumented code and view the traces in the Okahu IDE extension
Now when you [run](#running-the-application) the agent again, it'll generate Monocle trace that capture your agent's operations.
