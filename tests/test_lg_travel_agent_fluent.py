...
async def setup_supervior():
    ...
    global supervisor, flight_assistant, hotel_assistant
    supervisor, flight_assistant, hotel_assistant = await setup_agents(return_all_agents=True)
...
async def test_token_limit(monocle_trace_asserter: TraceAssertion):
    await monocle_trace_asserter.run_agent_async(supervisor, "langgraph", "Book a flight from Chennai to Mumbai on April 30th 2026. Book hotel Marriott in Central Mumbai.")
...
async def test_agent_and_tool_invocation(monocle_trace_asserter):
    await monocle_trace_asserter.run_agent_async(supervisor, "langgraph",
                    "Book a flight from San Francisco to Mumbai on April 30th 2026. Book hotel Marriott in Central Mumbai.")
    ...
    monocle_trace_asserter.called_agent("okahu_demo_lg_agent_lodging_assistant") \
        .contains_input("Book a flight from San Francisco to Mumbai on April 30th 2026. Book hotel Marriott in Central Mumbai.") \
        .contains_output("Marriott") \
        .contains_output("Central Mumbai") \
...
async def test_tool_invocation(monocle_trace_asserter):
    ...
    await monocle_trace_asserter.run_agent_async(supervisor, "langgraph",
                    "Book a flight from Chennai to Mumbai on April 30th 2026. Book hotel Marriott in Central Mumbai.")
